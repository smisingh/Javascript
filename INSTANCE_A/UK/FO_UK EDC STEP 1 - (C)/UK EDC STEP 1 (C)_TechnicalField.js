/**************		UK EDC STEP 1 (C) FO Technical Fields	*************
******************************************************************
...
Developer Name: Claire HEMERY
Date: 08/08/2017
Description: To modify popup mandatory field
******************************************************************
******************************************************************
Modified version - MOD-003
Developer Name: Claire HEMERY
Date: 09/08/2017
Description: To hide star when Effective date is prepopulated
******************************************************************
******************************************************************
Modified version - MOD-004
Developer Name: Claire HEMERY
Date: 10/08/2017
Description: Change the date format to avoid 0 before number > 10 
******************************************************************
*******************************************************************
Modified version - MOD-004
Developer Name: Benjamin CHESSON & Romain LECONTE
Date: 14/09/2017
Description: Change of mandatory fields and stard date contract
******************************************************************
*******************************************************************
Modified version - MOD-004
...
Date: 18/09/2017
Description:
	- 'Effective Date / Start Date' set to no Mandatory when star is hidden
	- Update function 'checkForm' : before submit the form enable fields probation status for saving
******************************************************************
*******************************************************************
...
window.disableFields = function(){
	console.log("disableFields function");
	//Action Reason
...
window.manageSubtopic = function(){
	...
	if(field.value == "0"){
		if(getSubtopic){
			if(field){
				if(getSubtopic.search(/job title/i) != -1){
					field.value = "2113";
				}else if(getSubtopic.search(/base location/i) != -1){
					field.value = "2114";
				}else if(getSubtopic.search(/working hours/i) != -1){
					field.value = "2115";
				}else if(getSubtopic.search(/Cost Centre/i) != -1){
					field.value = "2116";
				}else if(getSubtopic.search(/management team/i) != -1){
					field.value = "2117";
				}else if(getSubtopic.search(/Fixed Term/i) != -1){
					field.value = "2044";
				}else if(getSubtopic.search(/probation/i) != -1){
					field.value = "2050";
				}else{
...
window.defaultProbation = function(){
	...
	if(typeof probationCodeField !== undefined && probationCodeField !== null){
		if(typeof probationDescField !== undefined && probationDescField !== null){
			//Paste values
			$(probationCodeField).find('option[code*=227]').prop('selected',true);
            $(probationDescField).find('option[code*=228]').prop('selected',true);
            probationCodeField.setAttribute("readonly","true");
            probationCodeField.onmousedown = function(){return false;};
            probationDescField.setAttribute("readonly","true");
            probationDescField.onmousedown = function(){return false;};
		}else{
...
* MANAGE DELAI
**************/
window.setDelai = function(){
	var msg = "fonction setDelai : ";
	var subtopic = formulaire.INTERVENTIONS_EN_COURS$ELEMENT;
	var delai = formulaire.INTERVENTIONS_EN_COURS$DELAI;
	if(subtopic){
		if(delai){
			//Paste values
			if(subtopic.value == "2113"){
			//Job Title - 3 days
				delai.value = "2006";
			}else{
				//other subtopics - 2 days
				delai.value = "2005";
			}
		}else{
			msg += "champ "+probationDescField+" non trouvé";
		...
		msg += "champ subtopic read only or not found";
		console.log(msg);
	}
};
	...
	if(subtopic.value == "2050" || subtopic.value == "2044"){
		...
		var moisFin = dateFin.getMonth()+1;
	  	...
	  	//*************************************End MOD-004 *************************************
			...
			document.getElementById('divFieldINTERVENTIONS_EN_COURS_VALEUR5').getElementsByTagName("input")[0].style.display="none";
			document.getElementById('divFieldINTERVENTIONS_EN_COURS_VALEUR5').getElementsByTagName("img")[0].style.display="none";
			//*************************************Begin MOD-003 *************************************
			document.getElementById('divFieldINTERVENTIONS_EN_COURS_VALEUR5').getElementsByTagName("span")[0].style.display="none";
			//*************************************END MOD-003 *************************************
			//Rendre champ non obligatoire
			champObligatoire(formulaire.INTERVENTIONS_EN_COURS$VALEUR5, false);
			//finalDate.className = " ";
			//disableField(finalDate);
			//finalDate.parentNode.getElementsByTagName("img")[0].style.display = "none";
			var str = dateFinal;
			var div = document.getElementById('divFieldINTERVENTIONS_EN_COURS_VALEUR5');
			...
			var element = div1.querySelector('#lblmsg');
			if(!element)
			{
...
window.costCenterCheck = function () {
    var subtopicCheck = formulaire.INTERVENTIONS_EN_COURS$ELEMENT;
    if (typeof subtopicCheck !== undefined && subtopicCheck !== null) {
        if (subtopicCheck.value === "2116") {
            champObligatoire(formulaire.INTERVENTIONS_EN_COURS$VALEUR46, false);
        }
    }
};
...
window.fieldReadOnly = function() {
    document.getElementById("divFieldINTERVENTIONS_EN_COURS_VALEUR128").getElementsByTagName('input')[0].setAttribute('readonly', 'readonly');
    disableField(formulaire.INTERVENTIONS_EN_COURS$VALEUR128);
    ...
    document.getElementById("divFieldINTERVENTIONS_EN_COURS_VALEUR128").getElementsByTagName('input')[0].setAttribute('onkeypress', 'javascript:event.keyCode, [46] = false; return false;');
...
window.add_1day = function () {
    var date_initiale = document.getElementById('divFieldUTILISATEURS_CHAMPU186').getElementsByTagName('span')[0].innerHTML;
