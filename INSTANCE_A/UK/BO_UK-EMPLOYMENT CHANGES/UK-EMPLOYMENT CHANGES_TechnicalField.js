...
******************************************************************
Developer   - Claire HEMERY
Date	    - 04/08/2017
Change No   - MOD-003
Description - 1)Change custom field 24 to 25
******************************************************************
Developer   - Benjamin CHESSON
Date	    - 15/09/2017
Change No   - MOD-004
Description - Appel des fonctions setUpliftVisiblity et setBenefitVisiblity pour OnLoad
******************************************************************
Developer   - Pierrick Juy
Date	    - 04/10/2017
Change No   - MOD-005
Description - add custom fields VALEUR14, VALEUR16 to disable fields list
******************************************************************
Developer   - Romain Leconte
Date	    - 06/10/2017
Change No   - MOD-001 ++
Description - add verification for VALEUR78 and VALEUR89 fields
*******************************************************************
Modified version - MOD-006
Developer Name: Pierrick JUY
Date: 09/10/2017
Description:
	- update and add manageJobType Function onload to disable 'staff function' fields if needed
******************************************************************
Modified version - MOD-007
Developer Name: Pierrick JUY
Date: 27/10/2017
Description:
	- add controle for V1 and V2 on function manageSubGroup();
******************************************************************
Modified version - MOD-007a
Developer Name: Pierrick JUY
Date: 02/11/2017
	- change controle for 04 and 05 on function manageSubGroup();
******************************************************************/
...
window.copyValues = function () {
    ...
    copyValue("UTILISATEURS$CHAMPU74", "INTERVENTIONS_EN_COURS$VALEUR136");
};
...
//Change No   - MOD-003 20170804 modif custom field 24 to 27
...
window.disableFields = function () {
    ...
    disableField(formulaire.INTERVENTIONS_EN_COURS$VALEUR14);
    disableField(formulaire.INTERVENTIONS_EN_COURS$VALEUR15);
    disableField(formulaire.INTERVENTIONS_EN_COURS$VALEUR16);
    ...
    //Pay
    disableField(formulaire.INTERVENTIONS_EN_COURS$VALEUR86);
...
window.jobCodeToDesc = function () {
    ...
    if (jobCode) {
        if (jobType) {
            ...
            switch (jobCode.options[jobCode.selectedIndex].getAttribute('code')) {
                //Si les 2 champs concernés sont trouvés, on exécute la fonction
                case "176":
...
window.jobDescToCode = function () {
    ...
    if (jobCode) {
        if (jobType) {
            //Si les 2 champs concernés sont trouvés, on exécute la fonction
            switch (jobType.options[jobType.selectedIndex].getAttribute('code')) {
                case "188":
...
window.employeeCodeToDesc = function () {
    ...
    if (employeeCode) {
        if (employeeType) {
            //Si les 2 champs concernés sont trouvés, on exécute la fonction
            switch (employeeCode.options[employeeCode.selectedIndex].getAttribute('code')) {
                case "219":
...
window.employeeDescToCode = function () {
    ...
    if (employeeCode) {
        if (employeeType) {
            //Si les 2 champs concernés sont trouvés, on exécute la fonction
            switch (employeeType.options[employeeType.selectedIndex].getAttribute('code')) {
                case "223":
...
window.defaultProbation = function () {
    ...
    if (document.getElementById(probationCodeField)) {
        if (document.getElementById(probationDescField)) {
            ...
            //paste value
            $(probationCodeField).find('option[code*=' + probationCodeCode + ']').prop('selected', true);
            $(probationDescField).find('option[code*=' + probationDescCode + ']').prop('selected', true);
            //enable readOnly
...
window.calculateTargetVPNewHoursPeriod = function () {
    ...
    if (NewTargetVP) {
        if (NewTargetVPHoursPeriod) {
            ...
            console.log('NewTargetVPHoursPeriod is null');
        }
    } else {
        console.log('NewTargetVP is null');
    }
...
window.calculateTVPCurrtHoursPeriod = function(){
	var msg = "function calculateTVPCurrtHoursPeriod : ";
	...
	if (CurrTargetVP !== null || CurrTargetVP !== "undefined"){
		if(CurrTargetVPHoursPeriod){
			var d = new Date();
			...
			msg += "field CurrTargetVPHoursPeriod not found";
			console.log(msg);
		}
    }else{
		msg += "field CurrTargettVP not found";
		console.log(msg);
	}
};
...
function ListValue(customField) {
    //Verifie si le cv existe
    if (customField) {
        //Verifie si le cv est de type list
        if (customField.options) {
            console.log('EEE');
            return customField.options[customField.selectedIndex].getAttribute('code');
        }
        //Sinon si  si le cv est de type list
        else if (customField.value) {
            console.log('frfrfrEEE');
            return customField.value;
        } else if (customField.innerText) {
            return customField.innerText;
        }

    }
    console.log('The field' + customField + ' is missing');
    return -1;
}
...
window.manageSubgroup = function () {
	...
	if (typeOfChange) {
		if (subGroupCode) {
            if (subGroupDesc) {
				//161 : From Full Time to Part Time
                if (typeOfChange.options[typeOfChange.selectedIndex].getAttribute('code') == '161') {
                    switch (subGroupCode.value) {
                        case '01': //for Sal FT OT Inelig
                        case '02': //Sal PT OT Inelig 1                     
                            $(subGroupCodeNew).find('option[code*=220]').prop('selected', true);
                            $(subGroupDescNew).find('option[code*=224]').prop('selected', true);
                            break;
						//*************************************BEGIN Mod-007a ***********************************
                        case '04': //05 (501) - Sal PT OT Eligible 1 (226)
							$(subGroupCodeNew).find('option[code*=501]').prop('selected', true);
                            $(subGroupDescNew).find('option[code*=226]').prop('selected', true);
							break;
                        case '05': //Sal PT OT Elig 1  
                            $(subGroupCodeNew).find('option[code*=220]').prop('selected', true);
                            $(subGroupDescNew).find('option[code*=224]').prop('selected', true);
                            break;
						//*************************************BEGIN Mod-007 ***********************************
						case 'V1': //V2 (534) - Vice President PT (536)
							$(subGroupCodeNew).find('option[code*=534]').prop('selected', true);
                            $(subGroupDescNew).find('option[code*=536]').prop('selected', true);
							break;
						case 'V2': //V1 (533) - Vice President FT (535)
							$(subGroupCodeNew).find('option[code*=533]').prop('selected', true);
                            $(subGroupDescNew).find('option[code*=535]').prop('selected', true);
							break;
						default:
							break;
				...
				//162 : From Part Time to Full Time
                    switch (subGroupCode.value) {
                        case '01': //for Sal FT OT Inelig
                        case '02': //Sal PT OT Inelig 1                     
                            ...
                            break;
                        case '04': //for Sal FT OT Eligible
						//*************************************BEGIN Mod-007a ***********************************
                        case '05': //04 (222) - Sal FT OT Eligible (225)
...
window.checkSelectCode = function () {
    var targetSubTopic = document.getElementById('ELEMENTS');
    var targetSelect = document.getElementById('INTERVENTIONS_EN_COURS$VALEUR62');
    if (typeof targetSubTopic !== undefined && targetSubTopic !== null) {
        if (targetSubTopic.options.length != -1) {
            if (targetSubTopic.options[targetSubTopic.selectedIndex].getAttribute("value") == '2115') {
                if (typeof targetSelect !== undefined && targetSelect !== null) {
                    if (targetSelect.options.length != -1) {
                        if (targetSelect.options[targetSelect.selectedIndex].getAttribute("code") == '154') {
                            champObligatoire(formulaire.INTERVENTIONS_EN_COURS$VALEUR177, false);
                        } else {
                            champObligatoire(formulaire.INTERVENTIONS_EN_COURS$VALEUR177, true);
                        }
                    }
                } else {
                    console.log('Error in Function checkSelectCode, targetSelect is undefined or null');
                }
            } else {
                console.log('le code 2115 undefined');
            }
        } else {
        ...
        console.log('targetSubTopic not found');
    }
};
